...
/**
 * Adds metadata to a WorkRequest to identify what [CoroutineWorker] the [DelegatingWorker] should
 * delegate to [ReorderCategoriesWorker]
 */
internal fun KClass<out CoroutineWorker>.delegatedDataReorderCategories(
  eventsToHandle: List<Long>,
  categories: List<CategoryWithOrdinalIndex>,
) =
  Data.Builder()
    .putString(WORKER_CLASS_NAME, qualifiedName)
    .putLongArray(ReorderCategoriesWorker.KEY_NEW_ORDERED_CATEGORIES_IDS, eventsToHandle.toLongArray())
    .putString(ReorderCategoriesWorker.KEY_OLD_CATEGORIES, Json.encodeToString(categories))
    .build()
